...
const getters = {
  selectedItemsByIds: (state) => {
    ...
    for (const name in state.selected) {
      arrayOfIds[name] = state.selected[name].map(
        (item) => `${item.endpointType}_${item.id}`
      )
    }
...
const mutations = {
  [BROWSER.SAVE_ITEMS](state, items) {
    if (state.connector) {
      if (
        state.selected[state.connector] &&
        state.selected[state.connector].length
      ) {
  ...
  [BROWSER.DESTROY_ITEM](state, itemToDestroy) {
    if (state.selected[itemToDestroy.name]) {
      ...
      if (state.selected[itemToDestroy.name].length === 0)
        Vue.delete(state.selected, itemToDestroy.name)
