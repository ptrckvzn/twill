...
const state = {
  withPublicationToggle:
    window[process.env.VUE_APP_NAME].STORE.publication.withPublicationToggle ||
    false,
  published:
    window[process.env.VUE_APP_NAME].STORE.publication.published || false,
  publishSubmit:
    window[process.env.VUE_APP_NAME].STORE.publication.published ||
    !window[process.env.VUE_APP_NAME].STORE.publication.withPublicationToggle
      ? 'update'
      : 'live',
  publishedLabel:
    window[process.env.VUE_APP_NAME].STORE.publication.publishedLabel || 'Live',
  draftLabel:
    window[process.env.VUE_APP_NAME].STORE.publication.draftLabel || 'Draft',
  withPublicationTimeframe:
    window[process.env.VUE_APP_NAME].STORE.publication
      .withPublicationTimeframe || false,
  startDate:
    window[process.env.VUE_APP_NAME].STORE.publication.startDate || null,
  endDate: window[process.env.VUE_APP_NAME].STORE.publication.endDate || null,
  visibility:
    window[process.env.VUE_APP_NAME].STORE.publication.visibility || false,
  reviewProcess:
    window[process.env.VUE_APP_NAME].STORE.publication.reviewProcess || [],
  createWithoutModal:
    window[process.env.VUE_APP_NAME].STORE.publication.createWithoutModal ||
    false,
  saveType: undefined,
  visibilityOptions: [
    {
  ...
  submitDisableMessage:
    window[process.env.VUE_APP_NAME].STORE.publication.submitDisableMessage ||
    '',
  submitOptions: window[process.env.VUE_APP_NAME].STORE.publication
    .submitOptions || {
    draft: [
      {
        name: 'save',
...
const getters = {
  ...
  getSubmitOptions: (state) => {
    ...
    return state.published || !state.withPublicationToggle
      ? state.submitOptions[state.publishSubmit]
      : state.submitOptions.draft
  },
...
const mutations = {
  ...
  [PUBLICATION.UPDATE_PUBLISH_SUBMIT](state) {
    state.publishSubmit =
      state.published || !state.withPublicationToggle ? 'update' : 'live'
  },
