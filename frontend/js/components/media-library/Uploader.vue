<template>
  <div class="uploader">
    <div class="uploader__dropzone" ref="uploaderDropzone">
      <div class="button" ref="uploaderBrowseButton">
        {{ $trans('uploader.upload-btn-label', 'Add new') }}
      </div>
      <div class="uploader__dropzone--desktop">
        {{ $trans('uploader.dropzone-text', 'or drop new files here') }}
      </div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      replaceMedia: function (id) {
        ...
        const qqinputs =
          this.$refs.uploaderBrowseButton.querySelectorAll('[name = "qqfile"]')
      ...
      this._qqDropzone = new qq.DragAndDrop({
        ...
        callbacks: {
          ...
          processingDroppedFilesComplete:
            this._onProcessingDroppedFilesComplete.bind(this)
        }
      })
    },
    beforeDestroy() {
