        ...
        <template v-if="hasSingleMedia">
          ...
          <ul class="mediasidebar__metadatas">
            <li class="f--small" v-if="firstMedia.size">
              File size: {{ firstMedia.size | uppercase }}
            </li>
            <li
              class="f--small"
  ...
  export default {
    ...
    computed: {
      ...
      warningDeleteMessage: function () {
        const prefix = this.hasMultipleMedias
          ? this.allowDelete
            ? 'Some files are'
            : 'This files are'
          : 'This file is'
        return this.allowDelete
          ? prefix +
              " used and can't be deleted. Do you want to delete the others ?"
          : prefix + " used and can't be deleted."
      },
