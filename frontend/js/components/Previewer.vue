        ...
        <div class="previewer__inner">
          <div class="previewer__nav">
              ...
              >
                ...
                >
                  <template v-if="activeRevision">
                    {{ currentRevision.datetime | formatDate }} ({{
                ...
                <div slot="dropdown__content">
                  ...
                  >
                    ...
                    <span class="previewerRevision__datetime"
                      ><span class="tag" v-if="index === 0">{{
                        $trans('previewer.current-revision')
                      }}</span>
                      {{ revision.datetime | formatDate }}</span
                    >
                  </button>
                </div>
              </a17-dropdown>
            ...
            <ul class="previewer__breakpoints" v-if="!slipScreen">
              <li
                ...
                :key="breakpoint.size"
                class="previewer__breakpoint"
                :class="{ 's--active': activeBreakpoint === breakpoint.size }"
              >
            ...
            <div class="previewer__compare" v-if="activeRevision">
              <a href="#" v-if="!slipScreen" @click.prevent="compareView"
                ><span class="previewer__compareLabel">{{
                  $trans('previewer.compare-view')
                }}</span>
                <span v-svg symbol="revision-compare"></span
              ></a>
              <a href="#" v-else @click.prevent="singleView"
                ><span class="previewer__compareLabel">{{
                  $trans('previewer.single-view')
                }}</span>
                <span v-svg symbol="revision-single"></span
              ></a>
            </div>
          </div>
          ...
          <div class="previewer__content">
            <div class="previewer__iframe">
              <a17-iframe
                :content="activeRevision ? activeContent : currentContent"
                :size="activeBreakpoint"
                @scrollDoc="setIframeScroll"
                :scrollPosition="scrollPosition"
              ></a17-iframe>
            </div>
            <div class="previewer__iframe" v-if="slipScreen">
              <div class="previewer__iframeInfos">
                <span class="tag tag--revision">{{
                  $trans('previewer.current-revision')
                }}</span
                >{{ $trans('previewer.unsaved') }}
              </div>
              <a17-iframe
