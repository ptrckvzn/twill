<template>
  <span>
    ...
    <a
      ...
      :href="editWithLanguage(languages[0])"
      @click="editInPlace($event, languages[0])"
      class="more__languages f--small"
    >
      ...
      editWithQuery: function (context) {
        ...
        for (const prop in context) {
          if (context.hasOwnProperty(prop)) {
            queries.push(
              encodeURIComponent(prop) + '=' + encodeURIComponent(context[prop])
            )
          }
        }
