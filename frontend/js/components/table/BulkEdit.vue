    ...
    <div class="bulkEditor__inner" v-if="bulkIds.length">
      <div class="container">
        <p class="bulkEditor__infos">
          {{ bulkIds.length }} item{{ bulkIds.length > 1 ? 's' : '' }} selected
        </p>
        <div class="bulkEditor__dropdown">
          ...
          >
            <a17-button
              variant="dropdown"
              size="small"
              @click="$refs.bulkActionsDown.toggle()"
              >{{ $trans('listing.bulk-actions') }}</a17-button
            >

            <div slot="dropdown__content">
              <ul>
                <li>
                  <button v-if="bulkPublishable()" @click="bulkPublish">
                    {{ $trans('listing.dropdown.publish') }}
                  </button>
                  <button v-if="bulkPublishable(true)" @click="bulkUnpublish">
                    {{ $trans('listing.dropdown.unpublish') }}
                  </button>
                  <button v-if="bulkFeaturable()" @click="bulkFeature">
                    {{ $trans('listing.dropdown.feature') }}
                  </button>
                  <button v-if="bulkFeaturable(true)" @click="bulkUnFeature">
                    {{ $trans('listing.dropdown.unfeature') }}
                  </button>
                  <button v-if="bulkDeletable()" @click="bulkDelete">
                    {{ $trans('listing.dropdown.delete') }}
                  </button>
                  <button v-if="bulkRestorable()" @click="bulkRestore">
                    {{ $trans('listing.dropdown.restore') }}
                  </button>
                  <button v-if="bulkDestroyable()" @click="bulkDestroy">
                    {{ $trans('listing.dropdown.destroy') }}
                  </button>
                </li>
              </ul>
            </div>
        ...
        <a17-button variant="ghost" @click="clearBulkSelect">{{
          $trans('listing.bulk-clear')
        }}</a17-button>
      </div>
    </div>
  </div>
  ...
  export default {
            ...
            .reduce(
              (status, row) => {
                return {
                  ...
                  canFeature:
                    status.canFeature && row.hasOwnProperty('featured'),
                  published: status.published && (row.published || false),
                  canPublish:
                    status.canPublish && row.hasOwnProperty('published'),
                  deleted: status.deleted && (row.deleted || false),
                  canDelete: status.canDelete && row.delete !== null,
                  canDestroy:
                    status.canDestroy && row.hasOwnProperty('destroyable')
                }
              },
              {
                featured: true,
    ...
    methods: {
      bulkPublishable: function ($inverse = false) {
        return (
          window[process.env.VUE_APP_NAME].CMS_URLS.bulkPublish !== '' &&
          this.bulkStatus.canPublish &&
          ($inverse ? this.bulkStatus.published : !this.bulkStatus.published) &&
          !this.bulkStatus.deleted
        )
      },
      bulkFeaturable: function ($inverse = false) {
        return (
          window[process.env.VUE_APP_NAME].CMS_URLS.bulkFeature !== '' &&
          this.bulkStatus.canFeature &&
          ($inverse ? this.bulkStatus.featured : !this.bulkStatus.featured) &&
          !this.bulkStatus.deleted
        )
      },
      bulkDeletable: function () {
        return (
          window[process.env.VUE_APP_NAME].CMS_URLS.bulkDelete !== '' &&
          !this.bulkStatus.deleted &&
      ...
      bulkRestorable: function () {
        return (
          window[process.env.VUE_APP_NAME].CMS_URLS.bulkRestore !== '' &&
          this.bulkStatus.deleted
        )
      },
      bulkDestroyable: function () {
        return (
          window[process.env.VUE_APP_NAME].CMS_URLS.bulkDestroy !== '' &&
          this.bulkStatus.deleted &&
          this.bulkStatus.canDestroy
        )
      },
