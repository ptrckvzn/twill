  ...
  <tr class="activityRow">
    ...
    >
      <template v-if="isSpecificColumn(col)">
        <span
          v-if="col.name === 'published'"
          class="activityCell__pubstate"
          :class="{ 'activityCell__pubstate--live': row[col.name] }"
          :data-tooltip-title="row[col.name] ? 'Published' : 'Draft'"
          v-tooltip
        ></span>
        <!-- Published circle icon @click.prevent="togglePublish"  -->
        <a
          v-if="col.name === 'thumbnail'"
          class="activityCell__thumb"
          :href="row['edit']"
      ...
      <template v-else>
        <template v-if="col.name === 'name'">
          <a :href="row['edit']" class="activityCell__link">{{
            row[col.name]
          }}</a>
          <p class="activityCell__meta f--note">
            ...
            ></timeago>
            {{ $trans('dashboard.activity-row.by', 'by') }} {{ row['author'] }}
    ...
    <td class="activityCell activityCell--icon">
      <a17-dropdown ref="activityRowSetupDropdown" position="bottom-right">
        <a17-button
          variant="icon"
          @click="$refs.activityRowSetupDropdown.toggle()"
        ...
        <div slot="dropdown__content">
          <a
            ...
            target="_blank"
            >{{
              $trans('dashboard.activity-row.view-permalink', 'View Permalink')
            }}</a
          >
          <a v-if="row.hasOwnProperty('edit')" :href="row['edit']">{{
            $trans('dashboard.activity-row.edit', 'Edit')
          }}</a>
