...
export const stripOutBlockNamespace = (name, id) => {
  ...
  return nameWithoutBlock.match(/]/gi).length > 1
    ? nameWithoutBlock.replace(']', '')
    : nameWithoutBlock.slice(0, -1)
}
...
export const buildBlock = (block, rootState) => {
  return {
    ...
    blocks: Object.assign(
      {},
      ...Object.keys(rootState.repeaters.repeaters)
        .filter((repeaterKey) => {
          return repeaterKey.startsWith('blocks-' + block.id)
        })
        .map((repeaterKey) => {
          return {
            [repeaterKey.replace('blocks-' + block.id + '_', '')]:
              rootState.repeaters.repeaters[repeaterKey].map((repeaterItem) => {
...
export const isBlockEmpty = (blockData) => {
  return (
    isEmpty(blockData.content) &&
    isEmpty(blockData.browsers) &&
    isEmpty(blockData.medias) &&
    isEmpty(blockData.blocks)
  )
}
...
export const gatherRepeaters = (rootState) => {
  return Object.assign(
    ...
    ...Object.keys(rootState.repeaters.repeaters)
      ...
      .map((repeater) => {
        return {
          [repeater]: rootState.repeaters.repeaters[repeater].map(
            (repeaterItem) => {
...
export const getFormFields = (rootState) => {
  const fields = rootState.form.fields
    .filter((field) => {
      // we start by filtering out blocks related form fields
      return (
        !field.name.startsWith('blocks[') &&
        !field.name.startsWith('mediaMeta[')
      )
    })
    .reduce((fields, field) => {
...
export const getModalFormFields = (rootState) => {
  const fields = rootState.form.modalFields
    .filter((field) => {
      // we start by filtering out blocks related form fields
      return (
        !field.name.startsWith('blocks[') &&
